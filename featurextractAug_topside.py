...
dirName = 'C:\\Users\\tzhao\\Documents\\cameron_bop\\Seadrill\\westphoenix\\Event Logger Files\\parsedclean\\topside\\'
fileallcomm=dirName+'cleantopsideallcommand.csv'
...
#    get_feature(sing_comm, fd, allcommd)   pretime_thresh = 10, commtime_thresh = 180
def get_feature(sing_comm,df_allcom,df_wbpres, t_1_thresh = 10,dt_thresh = 180):
...
#==============================================================================
    ...
    if not preced_discard and not post_discard:    
        get_flag = True

#    
    if get_flag:
                #truncate analog data and prepare for return        
        dfw = df_wbpres[df_wbpres['AcqTime'].between(utc_now, utc_end)]
...
#        pres_mean,pres_len,pres_range = getFlowDetails(sing_comm, allcomm, wbpres,pretime_thresh, commtime_thresh) # check one command each time
        pres_mean,pres_len,pres_range = get_feature(sing_comm,allcommd,wbpres,pretime_thresh, commtime_thresh)
#        flow.append(fl)
#        timeOfFlow.append(tm)
        presmean.append(pres_mean)
        preslen.append(pres_len)
...
dirName = 'C:\\Users\\tzhao\\Documents\\cameron_bop\\Seadrill\\westphoenix\\Event Logger Files\\parsedclean\\topside\\'
dirNameOut = 'C:\\Users\\tzhao\\Documents\\cameron_bop\\Seadrill\\westphoenix\\featurecsv\\topside\\'
...
for func_ind in range(27,31):
#for func_ind in range(26,31):
...
#        Flow,TimeOfFlow,presmean,preslen,presrange,Active,NanType = getFlowList(df_wbpres,commData, allcomm, flowdata, pretime_thresh, commtime_thresh)
        # there might not be flow meter information for topside functions
#        Flow,TimeOfFlow,presmean,preslen,presrange = getFlowList(df_wbpres,commData, allcommd, flowdata, pretime_thresh, commtime_thresh)
